...
unsigned long onsite_syscall_entry;//ana and target share same syscall MSR in onsite mode
...
void imee_write_eoi_64 (void)
{
    ...
    {
        kvm_x86_ops->read_rsp_rip_64 ((u64*) &last_rsp, (u64*) &last_rip);
        kvm_x86_ops->get_sregs(vcpu, &imee_sregs);
        guest_syscall_entry = kvm_x86_ops->get_guest_syscall_entry(vcpu);
...
ulong gpa_to_hpa_guest (struct kvm* target_kvm, gpa_t gpa)
{
    ...
    list_for_each_entry(page, &arch->active_mmu_pages, link)
    {
        ...
        if (page->role.level == 1 && page->gfn == ((gpa >> 12) & ~0x1FFUL))
        {
            p = page->spt;
            idx = (gpa >> 12) & 0x1FFUL;
    ...
    list_for_each_entry(page, &arch->active_mmu_pages, link)
    {
        ...
        if (page->role.level == 2 && page->gfn == ((gpa >> 12) & ~0x3FFFFUL))
        {
            p = page->spt;
            idx = (gpa >> 21) & 0x1FFUL;
            if ((p[idx] & EPTE_L_MASK))
            {
                hpa = (ulong) (p[idx] & PAGE_MASK) + (gpa & 0x1FF000);
...
struct kvm_mmu_page* alloc_oasis_mmu_page (struct list_head* mmu_page_list, int lv, gfn_t gfn);
...
int adjust_ept_entry (intro_ctx_t* ctx, unsigned long gpa, unsigned long new_pa, int permission)
{
    ...
    if (!(pml4_ptr[pml4_idx] & EPTE_P_MASK))
    {
        lv = 3;
        gfn = gpa >> 12 & ~((1 << (lv*9)) -1); 
        oasis_mmu_page = alloc_oasis_mmu_page (&ctx->non_leaf_page, lv, gfn);
    ...
    if ((pt_ptr[pt_idx] & EPTE_P_MASK) && ((pt_ptr[pt_idx] & HPAE_MASK) != new_pa))
    {
...
static int fix_ept_mapping (intro_ctx_t* ctx, unsigned long new_pdpt_gpa)
{
    ...
    for (i = 0; i < 512; i ++)
    {
        if (imee_pdpt_ptr[i] & HPTE_P_MASK)
        {
            ...
            adjust_ept_entry (ctx, imee_pd_hpa, imee_pd_hpa, 0x7);// what if confliction
            ...
            for (j = 0; j < 512; j ++)
            {
                if (imee_pd_ptr[j] & HPTE_P_MASK)
                {
                    ...
                    adjust_ept_entry (ctx, imee_pt_hpa, imee_pt_hpa, 0x7);
                    imee_pt_ptr = __va(imee_pt_hpa);
                    for (k = 0; k < 512; k ++)
                    {
                        if (imee_pt_ptr[k] & HPTE_P_MASK)
                        {
                            imee_page_hpa = imee_pt_ptr[k] & HPAE_MASK;
                            // DBG ("k: %d, imee_page_hpa: %lx\n", k, imee_pt_ptr[k]);
                            ret = adjust_ept_entry (ctx, imee_page_hpa, imee_page_hpa, 0x7);
...
static int walk_gpt_new (intro_ctx_t* ctx, struct kvm_vcpu* vcpu, struct arg_blk* args)
{
    ...
    sec_pml4[kernel_idx] = sec_pml4[510] | NO_CONFLICT_GPA_MASK | 0x7;//generate non-conflit GPA for onsite PDPT page
    ...
    adjust_imee_vcpu (vcpu, dota_eip, dota_esp);
...
u64 make_imee_ept (intro_ctx_t* ctx)
{
    ...
    list_for_each_entry (cur, &ctx->pt_page, link)
    {
         ...
         if (pdpt[pdpt_idx] == 0)
         {
             lv = 2;
             gfn = cur->gfn & ~((1 << (lv*9)) -1); 
             oasis_mmu_page = alloc_oasis_mmu_page (&ctx->pd_page, lv, gfn);
         ...
         {
             printk ("pd entry is occupied. pd entry: %lx, pt page gfn: %lx.", pd[pd_idx], cur->gfn);
         }
    }
...
void copy_leaf_ept (intro_ctx_t* ctx, struct kvm_arch* arch)
{
    ...
    list_for_each_entry(t_page, &arch->active_mmu_pages, link)
    {
        ...
        // else if (t_page->role.level == 2)
        {
            DBG ("t PD page. gfn: %lx. cr0_wp: %x. \n", t_page->gfn, t_page->role.cr0_wp);
            for (i = 0; i < 512; i ++)
            {
                if ((t_page->spt[i] & EPTE_P_MASK) && (t_page->spt[i] & EPTE_L_MASK))
                {
                    break;
                }
            }
            if (i != 512)
            {
                ...
                for (i = 0; i < 512; i ++)
                {
                    ...
                    {
                        pd_ptr[i] = (t_page->spt[i] | 0x3) & ~0x4;// RW & NX
                        DBG ("\t i:%d -> %lX\n", i, t_page->spt[i]);
                    }
                }
            }
        ...
        else if (t_page->role.level == 3 && t_page->role.cr0_wp == 1)
        {
            ...
            for (i = 0; i < 512; i ++)
            {
                ...
                //     DBG ("\t i:%d -> %lX\n", i, t_page->spt[i]);
                if (t_page->spt[i] & EPTE_L_MASK)
                    printk (KERN_ERR "PDPTE 1GB page in target EPT: %lx\n", t_page->spt[i]);
            }
        }
    }
    return;
...
int adjust_ept_entry_s (intro_ctx_t* ctx, unsigned long gpa, ulong eptp, unsigned long new_pa)
{
    struct kvm_mmu_page* oasis_mmu_page;
    ...
    if (pml4_ptr[pml4_idx] == 0)
    {
        ...
        lv = 3;
        gfn = gpa >> 12 & ~((1 << (lv*9)) -1); 
        oasis_mmu_page = alloc_oasis_mmu_page (&ctx->s_non_leaf_page, lv, gfn);
        pdpt_ptr = oasis_mmu_page->spt;
        pml4_ptr[pml4_idx] = (__pa (pdpt_ptr) & HPAE_MASK) | 0x7; 
    }
    else 
...
static int fix_ept_mapping_s (intro_ctx_t* ctx)
{
    ...
    unsigned long pml4e, pdpte, pde, code_pte, data_pte, sys_code_pte, idt_pte, gdt_pte, tss_pte, tss1_pte;
    ...
    adjust_ept_entry_s (ctx, ctx->o_cr3, eptp, root_pt_pa);// the two EPTs share the same guest PML4, the pml4 entry is already adjusted in the setup stage of first EPT
    new_gpa = pml4e; 
...
void copy_s_leaf_ept (intro_ctx_t* ctx, struct kvm_arch* arch)
{
    ...
    list_for_each_entry (t_page, &arch->active_mmu_pages, link)
    {
        ...
        if (t_page->role.level == 1)
        {
            ...
            hlist_add_head(&oasis_mmu_page->hash_link, &imee_vcpu->kvm->arch.mmu_page_hash[(oasis_mmu_page->gfn & ((1 << KVM_MMU_HASH_SHIFT) - 1))]);
            
            pt_ptr = oasis_mmu_page->spt;
            for (i = 0; i < 512; i ++)
            {
                if (t_page->spt[i] & EPTE_P_MASK)
                    pt_ptr[i] = t_page->spt[i] | 0x2;//force to be W
            }
        }
        /* copy large PDE entries if any */
        // else if (t_page->role.level == 2)
        else if (t_page->role.level == 2 && t_page->role.cr0_wp == 1)
        {
            ...
            if (i != 512)
            {
                lv = 2;
                gfn = t_page->gfn;
                oasis_mmu_page = alloc_oasis_mmu_page(&ctx->s_pd_page, lv, gfn);
                ...
                for (i = 0; i < 512; i ++)
                {
                    if (t_page->spt[i] & EPTE_L_MASK)
                    {
                        pd_ptr[i] = t_page->spt[i] | 0x2;//force to be W
...
u64 make_imee_s_ept (intro_ctx_t* ctx)
{
    ...
    list_for_each_entry (cur, &ctx->s_pt_page, link)
    {
        ...
        if (pdpt[pdpt_idx] == 0)
        {
            lv = 2;
            gfn = cur->gfn & ~((1 << (lv*9)) -1); 
            oasis_mmu_page = alloc_oasis_mmu_page (&ctx->s_pd_page, 2, gfn);
            pd = oasis_mmu_page->spt;
            pdpt[pdpt_idx] = __pa (pd) | NL_EPTE_MASK;
        }
        else
        ...
        {
            printk ("ERROR s pd entry is occupied. pd entry: %lx, pt page gfn: %lx.", pd[pd_idx], cur->gfn);
        }
    }
...
int init_imee_vcpu (intro_ctx_t* next, struct kvm_vcpu* vcpu)
{
	...
	kvm_x86_ops->set_cr4(vcpu, (imee_sregs.cr4 | 0xa0 | 0x600 | 0x10000) & ~(0x360000)); // set PAE, PGE bit, and OSFXSR, OSXMMEXCPT bits for SSE, and FSGSBASE; clear SMAP, SMEP, OSXSAVE, PCIDE   
    imee_sregs.cr3 = next->o_cr3; 
    vcpu->arch.cr3 = next->o_cr3;
    kvm_x86_ops->write_cr3_64(imee_sregs.cr3);//set_cr3 provided by kvm would overwrite eptp 
    ...
    imee_gdt.size = 0x1000;//in IA-32e mode, a segment descriptor table can contain up to 8192 8-byte desciptors. 
    imee_gdt.address = imee_sregs.gdt.base;
    memcpy((void*)(&imee_tr), (void*)(&imee_sregs.tr), sizeof(struct kvm_segment));
...
ulong get_gpa_from_gva_guest (struct kvm* target_kvm, unsigned long gva, unsigned long g_cr3)
{
    int idx[4] = {
        (gva >> 39) & 0x1FF,
        (gva >> 30) & 0x1FF,
    ...
    for (lv = 0; lv < page_level; lv++)
    {
        ...
        else
        {
            ...
            {
                if (lv == 2)
                    return (gpte & GPA_MASK) + (gva & L_PDE_OFFSET);
                else if (lv == 1)
                    return (gpte & GPA_MASK) + (gva & L_PDPTE_OFFSET);
                else
                    return 0;
            }
        }
...
ulong get_hpa_from_gva_guest (struct kvm* target_kvm, unsigned long gva, unsigned long g_cr3)
{
    ulong hpa, gpa;
...
static void ana_idt_init (void)
{
    ...
    void* new_page;
    gate_desc s;
    ulong ana_pf_entry, ana_syscall_entry;
    ...
    onsite_syscall_entry = ana_syscall_entry + imee_arg.syscall_gate_addr + UK_OFFSET; 
    adjust_ept_entry (current_target, imee_arg.syscall_gate_pa, imee_arg.syscall_gate_pa, 0x3);//set ana syscall page as NX in a-EPT, so EPT violation to intercept ana syscall  
    ...
    pack_gate(&s, GATE_INTERRUPT, ana_pf_entry + imee_arg.ana_pf_c_page + UK_OFFSET, 0, 7, __KERNEL_CS);
    memcpy (new_page + 0x2*8*0xe, &s, 0x10);//analyzer's #PF hanlder
    adjust_ept_entry (current_target, imee_arg.t_idt_pa, virt_to_phys(new_page), 0x7);
    ...
    adjust_ept_entry (current_target, imee_arg.t_tss_pa, virt_to_phys(new_page), 0x7);
    new_page += g_tss_page_off;
    new_page += offsetof(struct x86_hw_tss, ist[6]);
    *((unsigned long*) new_page) = imee_arg.ana_pf_stack + 0x1000 + UK_OFFSET; //setup ana's #PF stack in IST[7]
    new_page = get_ept_page();//2nd TSS page 
    memset (new_page, 0xff, 0x1000);
    adjust_ept_entry (current_target, imee_arg.t_tss1_pa, virt_to_phys(new_page), 0x7);
    DBG ("new_page: %p, content: %lx. \n", new_page, *((unsigned long*) new_page));
    
    /* tune a-EPT for GDT */
    asm ("sgdt %0":"=m"(gdtr)::);
    host_gdt = (void*)(*(unsigned long*)(gdtr + 2));
    new_page = get_ept_page();
    memcpy (new_page, host_gdt, 0x1000);//setup onsite analyzer GDT based on host GDT
    set_tssldt_descriptor(&tss, imee_tr.base, DESC_TSS, 0x22c0);
    memcpy ((void*)(new_page + GDT_ENTRY_TSS*0x8), &tss, 0x10);//setup tr entry in GDT
    adjust_ept_entry (current_target, imee_arg.t_gdt_pa, virt_to_phys(new_page), 0x7);
    return;
}
...
static int t_idt_init(void)
{
    ...
    pg = pfn_to_page (pfn);
    pp = kmap_atomic (pg);
    memcpy ((void*)imee_arg.t_idt_va, pp, 0x1000);//set up t-IDT based on IDT in guest VM T
    pack_gate(&s, GATE_INTERRUPT, t_pf_entry + imee_arg.exit_gate_addr + UK_OFFSET, 3, 0, __KERNEL_CS);
    memcpy ((void*)(imee_arg.t_idt_va + 0x2*8*0xe), &s, 0x10);//set up #PF entry in t-IDT 
    DBG ("new #PF t-IDT entry: %lx. \n", *((unsigned long*)(imee_arg.t_idt_va+0x2*8*0xe)));
    adjust_ept_entry (current_target, imee_arg.ana_t_idt_pa, imee_arg.t_idt_pa, 0x3);// allow ana to RW target's IDT on a-EPT 

    /* TSS */
    memset ((void*)(imee_arg.t_tss_va), 0xff, 0x3000);//init io-bitmap, leave tss_struct to ana to setup in onsite mode;
    adjust_ept_entry (current_target, imee_arg.ana_t_tss_pa, imee_arg.t_tss_pa, 0x3);//allow ana to RW target's TSS_struct on a-EPT
    ...
    pg = pfn_to_page (pfn);
    pp = kmap_atomic (pg);
    memcpy ((void*)imee_arg.t_gdt_va, pp, 0x1000);//set up t-GDT based on GDT in guest VM
    set_tssldt_descriptor(&tss, imee_tr.base, DESC_TSS, 0x22c0);
    memcpy ((void*)(imee_arg.t_gdt_va + GDT_ENTRY_TSS*0x8), &tss, 0x10);//TSS entry
    adjust_ept_entry (current_target, imee_arg.ana_t_gdt_pa, imee_arg.t_gdt_pa, 0x3);//allow ana to RW target's GDT on a-EPT
    kunmap(pp);
...
int start_guest_intercept (struct kvm_vcpu *vcpu)
{
    ...
    {
        last_cr3 = imee_arg.hard_cr3;// use hardcoded cr3 instead of intercepted cr3
    }
    next->t_cr3 = last_cr3;
    ...
    if ((host_syscall_entry == 0) || (host_pf_entry == 0) || (guest_syscall_entry == 0))
    {
        printk ("host/guest symbol initialization failed. host_pf_entry: %lx. host_syscall_entry: %lx. guest_syscall_entry: %lx. \n", host_pf_entry, host_syscall_entry, guest_syscall_entry);
        return -1;
    }
    ...
    if (filp)
    {
        ...
        if (vcpu)
        {
            ...
            asm volatile("movq %%cr3, %%rax; \n\t"
                    "movq %%rax, %0; \n\t"
                    :"=m"(ana_h_cr3)::"%rax");
            kvm_x86_ops->write_host_cr3 (ana_h_cr3);//overwrite host cr3 in vmcs
            if (r >= 0)
            {
